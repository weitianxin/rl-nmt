        ...
        with self.graph.as_default():
            ...
            with tf.variable_scope("en_caption"):
                ...
                self.feature = tf.contrib.layers.batch_norm(inputs=self.image,
                                            decay=0.95,
                                            center=True,
                                            scale=True,updates_collections=None,is_training=False)#self.is_training
                ...
                sentence = tf.nn.embedding_lookup(self.lookup_table,tf.ones(batch_size,dtype=tf.int32)*2)
                if not is_training:
                    beam_width = 5
                    self.feature = tf.tile(tf.expand_dims(self.feature,axis=1),[1,beam_width,1,1])
                    self.preds = self.beam_search(sentence,beam_width=beam_width,num_classes=len(en2idx))
                else:
                    for i in range(hp.maxlen):
                        ...
                        sentence = tf.nn.embedding_lookup(self.lookup_table,self.x[:,i])
                        last_state = state
                        last_output = output
            if is_training:
                ...
                self.loss = tf.nn.softmax_cross_entropy_with_logits_v2(labels=tf.one_hot(self.x_target,len(en2idx)),logits=logits)
        ...
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
            ...
            for epoch in range(hp.num_epochs):
                for i in range(num_batch):
                    ...
                    if (step+1)%1000==0:
                        saver.save(sess,save_path=hp.logdir_cap_en+ '/model_step_%d'%(step))
