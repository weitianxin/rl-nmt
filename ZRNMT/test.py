...
if __name__ == '__main__':
    ...
    save_list = os.listdir("{}".format(mode))#mode
    ...
    print("Graph loaded")
    # Load vocabulary
    x_val,Targets_val,idents = load_test_rl_data(set="val",task=task,language=mode[:2])
    num_batch_val = int(math.ceil(len(x_val) / hp.batch_size_test))

    # prepare ref file
    if task == "task2":
        f_ref1 = open("result/{}_ref_1".format(mode), "w+")
        f_ref2 = open("result/{}_ref_2".format(mode), "w+")
        f_ref3 = open("result/{}_ref_3".format(mode), "w+")
        f_ref4 = open("result/{}_ref_4".format(mode), "w+")
        f_ref5 = open("result/{}_ref_5".format(mode), "w+")
    ...
    with g.graph.as_default():
        for i,file in enumerate(file_list):
            with tf.Session() as sess:
                ...
                saver.restore(sess,"{}".format(mode)+file)#mode
                print("restore")
                f_hypo = open("result/{}_hypo_{}".format(mode, save_num), "w+")
                save_num += 1
