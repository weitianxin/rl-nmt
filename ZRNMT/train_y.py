...
class Graph():
    def __init__(self, is_training=True,beam_width=5,mode="de2en"):
        ...
        with self.graph.as_default():
            ...
            if is_training:
                ...
                self.prob_z2y = -self.image_caption(self.y_istarget, self.y_target, set="{}_caption".format(mode[-2:]),
                                                   flag=1)
            else:
            ...
            if is_training:
                ...
                # optimize
                lamda = 1.0
                self.loss_rl = tf.reduce_sum(self.reward*self.prob_z2y*self.sample_index)/tf.reduce_sum(self.y_istarget*
                ...
                self.loss_nmt = tf.reduce_sum(self.prob_x2y*self.sample_index)/tf.reduce_sum(self.y_istarget*
                                                                                        tf.expand_dims(self.sample_index,axis=1))
                self.loss_fix = tf.reduce_sum(lamda*self.prob_z2y*self.not_sample_index)/tf.reduce_sum(self.y_istarget*
    ...
    def image_caption(self, mask, preds, set,flag=1):
        with tf.variable_scope(set,reuse=tf.AUTO_REUSE):
            ...
            if flag==0:
                self.sample = self.beam_search_image(sentence, beam_width=self.beam_width, num_classes=len(self.decoder_2idx))
                return self.sample
            else:
...
def train():
    ...
    with tf.Session(config=config, graph=g.graph) as sess:
        ...
        if mode == "de2en":
            ...
            saver2.restore(sess, eval("hp.logdir_cap_{}".format(mode[-2:])) + "/model_step_9999")
        print("Restored!")
        # lr = hp.lr
        num = 0
