...
class Graph():
    def __init__(self, is_training=True):
        ...
        with self.graph.as_default():
            ...
            if is_training:
                ...
                self.preds = self.beam_search(self.y,beam_width=5,num_classes=len(de2idx))
            ...
            if is_training:
                ...
                self.acc = tf.reduce_sum(tf.to_float(tf.equal(self.preds, self.y_target)) * self.istarget) / (
                    tf.reduce_sum(self.istarget))
                tf.summary.scalar('acc', self.acc)
                # Loss
                self.y_smoothed = label_smoothing(tf.one_hot(self.y_target, depth=len(de2idx)))
                ...
                for i in range(hp.num_blocks):
                    with tf.variable_scope("num_blocks_{}".format(i)):
                        ...
                        self.dec = feedforward(self.dec, num_units=[4 * hp.hidden_units, hp.hidden_units],
                                               reuse=tf.AUTO_REUSE)
                dec1 = tf.layers.dense(self.dec, len(self.de2idx))
    ...
    def beam_search(self, sentence, beam_width, num_classes):
        ...
        for i in range(hp.maxlen - 1):
            ...
            value, index = tf.nn.top_k(tf.reshape(sum_logprob, [self.batch_size, beam_width * num_classes]),
                                       k=beam_width)  # batch x beam
...
if __name__ == '__main__':
    ...
    x, y, y_target = load_train_data(mode="en2de")
    num_batch = int(math.ceil(len(x)/hp.batch_size))
    smoothie = SmoothingFunction().method2
    # Start session
    if not os.path.exists(hp.logdir_en2de): os.mkdir(hp.logdir_en2de)
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(hp.num_epochs):
                for i in range(num_batch):
                    ...
                    # val
                    if (step + 1) % 1000 == 0:
                        saver.save(sess, save_path=hp.logdir_en2de + '/model_epoch_%d' % step)
    print("Done")
